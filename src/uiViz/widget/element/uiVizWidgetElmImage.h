...
{
    class uiVizWidgetElmImage : public uiVizWidgetElm
    {
    public:
        uiVizWidgetElmImage(string persistentId, string widgetXML, uiVizWidgetBase *parent) : uiVizWidgetElm(persistentId, widgetXML, parent)
        {
            initWidget();
            loadState(widgetXML);
        }
        ...
        {
            ofxXmlSettings settings = uiVizWidgetElm::saveState();
