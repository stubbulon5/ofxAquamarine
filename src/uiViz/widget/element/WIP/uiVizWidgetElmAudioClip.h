...
{
    class uiVizWidgetElmAudioClip : public uiVizWidgetElm
    {

    public:
        uiVizWidgetElmAudioClip(string persistentId, string widgetXML, uiVizWidgetBase *parent) : uiVizWidgetElm(persistentId, widgetXML, parent)
        {
            initWidget();
            loadState(widgetXML);
        }
        ...
        virtual ~uiVizWidgetElmAudioClip()
        {
        }
    ...
    private:
        uiVizElm vizElm_AUDIO_CLIP;
        uiVizElm vizElm_AUDIO_CLIP_FOCUS;
        string mClipFile = "";
