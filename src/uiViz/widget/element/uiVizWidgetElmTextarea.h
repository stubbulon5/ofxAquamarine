...
{
    class uiVizWidgetElmTextarea : public uiVizWidgetElm
    {

    public:
        uiVizWidgetElmTextarea(string persistentId, string widgetXML, uiVizWidgetBase *parent) : uiVizWidgetElm(persistentId, widgetXML, parent)
        {
            initWidget();
            loadState(widgetXML);
        }
        ...
        virtual ~uiVizWidgetElmTextarea()
        {
        }
        ...
        virtual void update(uiVizWidgetContext context) override
        {
            ...
            setContentBoundsScaled(
                uiVizCoord::vizBounds(r.x, r.y, r.width, r.height));
        }
