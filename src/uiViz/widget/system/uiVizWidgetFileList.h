...
#include <stdio.h>
    ...
    */
    ...
    class uiVizWidgetFileList : public uiVizWidgetTable
    {
    public:
        uiVizWidgetFileList(string persistentId, string widgetXML) : uiVizWidgetTable(persistentId, widgetXML)
        {
            initWidget();
            loadState(widgetXML);
        }
        ...
        virtual ofxXmlSettings saveState() override
        {
            ...
            settings.setAttribute("widget", "class", uiVizWidgetManager::WIDGET_CLASS_FILE_LIST, 0);
        ...
        void setPath(string val)
        {
            ...
            vector<uiVizWidgetTableRow> dir_rows = vector<uiVizWidgetTableRow>();
            vector<uiVizWidgetTableRow> file_rows = vector<uiVizWidgetTableRow>();
            ...
            else if (mPath == "[GOOGLE_DRIVE]")
            {
                ...
                */
                ...
                for (ofFile file : files)
                {
                    ...
                    {
                        row.cells.push_back(
                            uiVizWidgetTableCell(file.isDirectory() ? "DIR" : "FILE", ""));
                    }

                    row.cells.push_back(
                        uiVizWidgetTableCell(fileName, fileName));

                    string fileSize = "";
                    if (!file.isDirectory())
                        fileSize = uiVizShared::Pretty_Bytes(file.getSize());

                    if (getShowFileSize())
                    {
                        row.cells.push_back(
                            uiVizWidgetTableCell("", file.isDirectory() ? "" : fileSize));
                    }
            ...
            else
            {
                ...
                catch (...)
                {
                    ...
                    {
                        row.cells.push_back(
                            uiVizWidgetTableCell("Access denied", ""));
                    }

                    row.cells.push_back(
                        uiVizWidgetTableCell("DIR", "Access denied"));
                ...
                if (getMediaMode())
                {
                    ...
                    for (ofFile file : files)
                    {
                        ...
                        if (file.isDirectory() || file.isDevice())
                        {
                            ...
                            uiVizWidgetTableCell c = uiVizWidgetTableCell(fullFilePath, (mPath == "/" ? fullFilePath : fileName), "center");
                        ...
                        else
                        {
                            ...
                            uiVizWidgetTableCell c = uiVizWidgetTableCell(fullFilePath, "", "center");
                ...
                else
                {
                    ...
                    for (ofFile file : files)
                    {
                        ...
                        row.cells.push_back(
                            uiVizWidgetTableCell(fileName, (mPath == "/" ? fullFilePath : fileName)));
            ...
            std::vector<uiVizWidgetTableRow> rows(dir_rows);
            rows.insert(rows.end(), file_rows.begin(), file_rows.end());

            setHeaderRow(uiVizWidgetTableRow("FILES", headerCells));
            setTableRows(rows);
        ...
        { // type = [DIR|FILE]
            ...
            uiVizWidgetTableRow row(rowKey, cells);
            row.key = key;
        ...
        virtual void drawCellContent(bool isRowSelected, bool isRowHovered, bool isCellHovered, int absoluteScaledX, int absoluteScaledY, int absoluteScaledLabelX, int absoluteScaledLabelY, int scaledWidth, int scaledHeight, uiVizWidgetTableRow &row, uiVizWidgetTableCell &cell, int rowIndex, int colIndex)
        {
            ...
            if (getMediaMode())
            {
                ...
                {
                    uiVizIcon img = uiVizIcon(
                        cell.key,
                        uiVizCoord::vizBounds(absoluteScaledX + scale(10), absoluteScaledY + scale(10), scaledWidth - scale(20), scaledHeight - scale(20)),
                ...
                {
                    uiVizWidgetTable::drawCellContent(isRowSelected, isRowHovered, isCellHovered, absoluteScaledX, absoluteScaledY, absoluteScaledLabelX, absoluteScaledLabelY, scaledWidth, scaledHeight, row, cell, rowIndex, colIndex);
                }
            }
            else
            {
        ...
        virtual void drawRowHighlight_Hover(bool isRowSelected, bool isRowHovered, int absoluteScaledX, int absoluteScaledY, int scaledWidth, int scaledHeight, uiVizWidgetTableRow &row, int rowIndex)
        {
            ...
            {
                uiVizWidgetTable::drawRowHighlight_Hover(isRowSelected, isRowHovered, absoluteScaledX, absoluteScaledY, scaledWidth, scaledHeight, row, rowIndex);
            }
        }
    ...
    private:
        ...
        uiVizIcon iconFile, iconDir, iconDrag;
