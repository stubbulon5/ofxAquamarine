...
//  bloom-macos
//
//  Created by Stuart Barnes on 11/012/2018.
//
#pragma once
#include "ofMain.h"
#include "ofxSvg.h"
...
namespace Aquamarine
{
    ...
    public:
        ...
        uiVizIcon() : mPath(""), mScaledBounds(uiVizCoord::vizBounds(0, 0, 0, 0)), mOriginalScaledBounds(uiVizCoord::vizBounds(0, 0, 0, 0)), mScale(1.0f), mIconSize(uiVizIcon::IconSize::REGULAR), mColor(ofColor::white), mTag(0) {}

        uiVizIcon(string path, uiVizCoord::vizBounds scaledBounds, float scale, uiVizIcon::IconSize iconSize, ofColor color, int tag, string title)
        ...
        {
            uiVizIcon(path, scaledBounds, scale, iconSize, color, tag);
        }

        uiVizIcon(string path, uiVizCoord::vizBounds scaledBounds, float scale, uiVizIcon::IconSize iconSize, ofColor color, int tag)
            : mPath(path), mScaledBounds(scaledBounds), mOriginalScaledBounds(scaledBounds), mScale(scale), mIconSize(iconSize), mColor(color), mTag(tag)
        {
            ...
            try
            {
                ...
                {
                    ofLogError("uiVizIcon") << "load(): path does not exist: \"" << mPath << "\"\n";
                }
            }
            catch (...)
            {
        ...
        void reload(float scale, uiVizCoord::vizBounds scaledBounds)
        {
        ...
        uiVizCoord::vizBounds getScaledBounds()
        {
        ...
        {
            mScaledBounds = uiVizCoord::vizBounds(x, y, width, height);
        ...
        {
            mScaledBounds = uiVizCoord::vizBounds(x, y, mScaledBounds.width, mScaledBounds.height);
        }
        ...
        void setScaledSize(int width, int height)
        {
            ...
            mScaledBounds = uiVizCoord::vizBounds(mScaledBounds.x, mScaledBounds.y, width, height);
        }
    ...
    private:
        ...
        int mTag;
        uiVizCoord::vizBounds mScaledBounds;
        uiVizCoord::vizBounds mOriginalScaledBounds;
        float mScale = 1.0f;
        uiVizIcon::IconSize mIconSize;
        ofColor mColor;
    };

    class uiVizIconCache
    {
    ...
    public:
        ...
        static void bootstrapIconMap(float scale, std::map<std::string, uiVizIcon *> map);
        static uiVizIcon getIcon(string iconKey);
        static uiVizIcon &getIconRef(string iconKey);
        static std::map<std::string, uiVizIcon *> mIconCacheMap;
    };
