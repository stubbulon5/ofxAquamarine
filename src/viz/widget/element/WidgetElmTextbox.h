...
{
    class WidgetElmTextbox : public WidgetElm
    {
    ...
    public:
        ...
        WidgetElmTextbox(string persistentId, string widgetXML, WidgetBase *parent) : WidgetElm(persistentId, widgetXML, parent)
        {
            initWidget();
            loadState(widgetXML);
        }
     ...
     */
        ...
        void calculateUsableY()
        {
            ...
            mUsableY = WidgetBase::getUsableY() + getNonScaledPadding() + (laidOutLines.size() > 0 ? deScale(laidOutLines[0].lineH) : 0);
        }
